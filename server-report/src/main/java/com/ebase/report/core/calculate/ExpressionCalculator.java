...
public class ExpressionCalculator {
     ...
     * 计算主逻辑
     */
    private static double _cal(String expression, Stack<OperatorEnum> opStack, Stack<BigDecimal> dataStack) throws ExpressionFormatException {
        ...
        for (int i = 0; i < length; ) {
            ...
            if (Character.isDigit(value) || NEGATIVE_SIGN == value) {
                ...
                if (!OperatorEnum.RIGHT_BRACKET.equals(curOp = OperatorEnum.getByExpression(String.valueOf(value)))
                        && (opStack.isEmpty() || OperatorEnum.LEFT_BRACKET.equals(stackOp = opStack.peek()) || stackOp.getPriority() < curOp.getPriority())) {
                    opStack.push(curOp);
                    ++i;
                } else {
                    singleCal(opStack, dataStack);
                    if (OperatorEnum.RIGHT_BRACKET.equals(curOp)) {
                ...
                curOp = OperatorEnum.getByExpression(strBuilder.toString().toLowerCase());
                if (null == curOp) {
                ...
                if (params.size() != curOp.getParamNumber())
                    throw new ExpressionFormatException("公式，参数个数不正确:[运算符号:" + curOp + "][需要参数个数:" + curOp.getParamNumber() + "][入参:" + expression + "]");
                for (String param : params) {
                    dataStack.push(new BigDecimal(new ExpressionCalculator().cal(param)));
                }
     ...
     */
    private static void singleCal(Stack<OperatorEnum> opStack, Stack<BigDecimal> dataStack) throws ExpressionFormatException {
        OperatorEnum op = opStack.peek();
        if (OperatorEnum.LEFT_BRACKET.equals(op)) {
            return;
        }
        opStack.pop();
